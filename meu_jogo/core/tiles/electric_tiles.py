...
class SandTile(Tile):
    ...
    def draw(self, surface, grid_pos, size, camera_offset):
        ...
        for i in range(5):
            px2 = rect.x + (i * 6 + gx * 4) % (size - 2)
            py2 = rect.y + (i * 5 + gy * 3) % (size - 2)
...
class ChargedSandTile(Tile):
    ...
    def draw(self, surface, grid_pos, size, camera_offset):
        ...
        for i in range(2):
            if math.sin(t * 8.0 + i * 1.5 + gx * 0.7 + gy * 0.4) > 0.6:
                lx = rect.x + (i * 11 + gx * 3) % (size - 8) + 4
                ly = rect.y + (i * 8 + gy * 5) % (size - 8) + 4
...
class LightningCrystalTile(Tile):
    ...
    def draw(self, surface, grid_pos, size, camera_offset):
        ...
        t = pygame.time.get_ticks() / 1000.0
        glow = int(150 + 105 * abs(math.sin(t * 4.0 + gx + gy)))
...
class NeonBorderTile(Tile):
    ...
    def draw(self, surface, grid_pos, size, camera_offset):
        ...
        t   = pygame.time.get_ticks() / 1000.0
        gx, gy = int(grid_pos.x), int(grid_pos.y)
        gv  = int(100 + 155 * abs(math.sin(t * 2.0 + gx * 0.5 + gy * 0.4)))
...
class BoltTile(Tile):
    ...
    def draw(self, surface, grid_pos, size, camera_offset):
        ...
        t = pygame.time.get_ticks() / 1000.0
        gx, gy = int(grid_pos.x), int(grid_pos.y)
        glow = int(90 + 80 * abs(math.sin(t * 2.2 + gx * 0.5 + gy * 0.4)))
