...
class LavaTile(Tile):
    ...
    def draw(self, surface, grid_pos, size, camera_offset):
        ...
        for i in range(3):
            ...
            phase = math.sin(t * 2.5 + i * 1.2 + gx * 0.4 + gy * 0.3)
...
class HotRockTile(Tile):
    ...
    def draw(self, surface, grid_pos, size, camera_offset):
        ...
        t = pygame.time.get_ticks() / 1000.0
        for i in range(3):
            cx2 = rect.x + (i * 9 + gx * 5) % (size - 6) + 3
            cy2 = rect.y + (i * 7 + gy * 4) % (size - 6) + 3
            glow = int(100 + 100 * abs(math.sin(t * 1.5 + i + gx * 0.2)))
...
class AshTile(Tile):
    ...
    def draw(self, surface, grid_pos, size, camera_offset):
        ...
        pygame.draw.rect(surface, self.color, rect)
        for i in range(6):
            px2 = rect.x + (i * 5 + gx * 3) % (size - 2)
            py2 = rect.y + (i * 4 + gy * 5) % (size - 2)
...
class EmberTile(Tile):
    ...
    def draw(self, surface, grid_pos, size, camera_offset):
        ...
        t = pygame.time.get_ticks() / 1000.0
        for i in range(4):
            bx = rect.x + (i * 7 + gx * 4) % (size - 4) + 2
            by = rect.y + (i * 5 + gy * 3) % (size - 4) + 2
            pulse = abs(math.sin(t * 3.0 + i + gx * 0.5))
            c = int(180 + 75 * pulse)
...
class LavaDropTile(Tile):
    ...
    def draw(self, surface, grid_pos, size, camera_offset):
        ...
        t = pygame.time.get_ticks() / 1000.0
        for i in range(2):
            phase  = (t * 1.2 + i * 0.6 + gy * 0.4) % 1.0
            dx2    = (i * 11 + gx * 7) % (size - 8) + 4
...
class BoneTile(Tile):
    ...
    def draw(self, surface, grid_pos, size, camera_offset):
        ...
        t = pygame.time.get_ticks() / 1000.0
        glow = int(55 + 40 * abs(math.sin(t * 1.5 + gx * 0.4 + gy * 0.3)))
        cx, cy = rect.centerx, rect.centery
